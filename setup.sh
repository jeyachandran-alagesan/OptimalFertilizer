...
echo "==================================================="
echo " Optimal Fertilizer Prediction - setup"
...
if ! command -v "$PYTHON_BIN" >/dev/null 2>&1; then
  ...
  echo "Please install Python 3.11 (e.g., via pyenv or system packages) and try again."
  exit 1
fi
...
python -m ipykernel install --user --name "$KERNEL_NAME" --display-name "Python 3.11 (Fertilizer)"
...
echo "  - notebooks/01-OptimalFertilizer_Approach1.ipynb (Ensemble with EDA - trains models)"
echo "  - notebooks/02-OptimalFertilizer_Approach2.ipynb (XGBoost with Feature Engineering)"
echo "  - notebooks/03-OptimalFertilizer_GUI.ipynb (Interactive GUI Application)"
echo ""
echo "Note: Run Approach1 first to generate model files for the GUI"
echo ""
